...
ForEach ($AppxPackage in $Apps) {
    If ((Get-AppxPackage -AllUsers -Name $AppxPackage) -or (Get-AppxProvisionedPackage -Online | Where-Object DisplayName -like $AppxPackage)) {
        Write-Host "Trying to remove $AppxPackage from ALL users..."
        ...
        Write-Host "$AppxPackage was already removed or not found..." -ForegroundColor Yellow
    }
}
